...
namespace UHF
{
	...
	public class Read_Fragment : Fragment
	{
		...
		private void read() {
			...
			Com.Rscja.Deviceapi.RFIDWithUHF.BankEnum bank=Com.Rscja.Deviceapi.RFIDWithUHF.BankEnum.ValueOf (spnBank_R.SelectedItem.ToString ());
               ...
               // string entity = mContext.uhfAPI.ReadDataEx(pwdStr,                bank, int.Parse(ptrStr), int.Parse(cntStr));
                string entity = mContext.uhfAPI.ReadData(pwdStr, RFIDWithUHF.BankEnum.Uii, 32, 0, "00", bank, int.Parse(ptrStr), int.Parse(cntStr));
    ...
    //            SimpleRFIDEntity entity = mContext.uhfAPI.ReadData(pwdStr, bank, int.Parse(ptrStr), int.Parse(cntStr));
    //            if (entity!=null) 
			 //   {
				    
				//    edtTxtData_R.Text = entity.Data;
			 //   	Sound ();
				//}
			    else
			    {
